...
test.describe('Session Management', () => {
  ...
  test('should persist session across page reloads', async ({ page }) => {
    ...
    await expect(page.locator('text=tier2').or(page.locator('text=Tier 2'))).toBeVisible();
  });
  ...
  test('should sync authentication across multiple tabs', async ({ browser }) => {
    ...
    await page1.reload();
    await expect(page1.locator('button:has-text("ÀÃ·åÜüÉx")')).toBeVisible();

    // Tab 2 should also show authenticated state
    await page2.goto('/');
    await expect(page2.locator('button:has-text("ÀÃ·åÜüÉx")')).toBeVisible();

    // Logout in tab 1
    await page1.click('button:has-text("í°¢¦È")');
    ...
    await page2.reload();
    await expect(page2.locator('button:has-text("í°¤ó")')).toBeVisible();
  ...
  test('should maintain session info in dashboard', async ({ page }) => {
    ...
    await expect(page.locator('text=tier3').or(page.locator('text=Tier 3'))).toBeVisible();
    
    // Check rate limit display based on tier
    const rateLimitText = page.locator('text=/300Þ\\/|300 requests per minute/i');
