...
test.describe("Access Control", () => {
  ...
  test("should allow access to public routes without auth", async ({
    ...
    for (const route of publicRoutes) {
      ...
      if (route === "/") {
        ...
        await expect(page.locator('h1:has-text("�������ЛAPI")')).toBeVisible();
        await expect(page.locator('button:has-text("����")')).toBeVisible();
  ...
  test("should display tier-based information correctly", async ({ page }) => {
    ...
    const tiers = [
      { name: "tier1", rateLimit: "60�/" },
      { name: "tier2", rateLimit: "120�/" },
      { name: "tier3", rateLimit: "300�/" },
    ];
  ...
  test("should display appropriate UI based on auth state", async ({
    page,
  }) => {
