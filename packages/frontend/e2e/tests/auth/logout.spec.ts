...
test.describe('Logout Flow', () => {
  test('should logout successfully from landing page', async ({ page, context }) => {
  ...
  test('should logout successfully from dashboard', async ({ page, context }) => {
    ...
    await expect(page.locator('text=logout-dashboard@example.com')).toBeVisible();
  ...
  test('should clear all authentication data on logout', async ({ page, context }) => {
    ...
    await page.evaluate(() => {
      ...
      localStorage.setItem('supabase.auth.expires_at', String(mockSession.expires_at));
      localStorage.setItem('supabase.auth.refresh_token', mockSession.refresh_token);
    ...
    const dataAfterLogout = await page.evaluate(() => {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && (key.includes('auth') || key.includes('supabase'))) {
          authKeys.push({ storage: 'local', key, value: localStorage.getItem(key) });
        }
      }
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        ...
        if (key && (key.includes('auth') || key.includes('supabase'))) {
          authKeys.push({ storage: 'session', key, value: sessionStorage.getItem(key) });
        }
      }
  ...
  test('should update UI immediately after logout', async ({ page }) => {
    ...
    const dashboardButton = page.locator('button:has-text("ÀÃ·åÜüÉx")');
    const logoutButton = page.locator('button:has-text("í°¢¦È")');
    const loginButton = page.locator('button:has-text("í°¤ó")');
    const signupButton = page.locator('button:has-text("µ¤ó¢Ã×")');
