...
export const getOptimizationSuggestions = (): string[] => {
  ...
  if (metrics.LCP && metrics.LCP > 2500) {
    suggestions.push('LCPが2.5秒を超えています。画像の最適化やサーバーレスポンスの改善を検討してください。');
  }

  // CLS（Cumulative Layout Shift）が高い場合
  if (metrics.CLS && metrics.CLS > 0.1) {
    suggestions.push('CLSが0.1を超えています。画像やiframeのサイズを明示的に指定してください。');
  }

  // INP（Interaction to Next Paint）が長い場合
  if (metrics.INP && metrics.INP > 200) {
    suggestions.push('INPが200msを超えています。JavaScriptの実行を最適化してください。');
  }
