...
describe("useApiData with MSW", () => {
  it("should fetch data successfully", async () => {
    const { result } = renderHook(() =>
      useApiData("/api/secure/123/test.json")
    );
  ...
  it("should handle 404 errors", async () => {
    const { result } = renderHook(() =>
      useApiData("/api/secure/404/notfound.json")
    );
  ...
  it("should handle rate limit errors", async () => {
    ...
    const { result } = renderHook(() =>
      useApiData("/api/secure/test/rate-limit.json")
    );
  ...
  it("should handle network errors", async () => {
    ...
    const { result } = renderHook(() =>
      useApiData("/api/secure/test/network-error.json")
    );
  ...
  it("should handle custom response data", async () => {
    ...
    const { result } = renderHook(() =>
      useApiData("/api/secure/custom/data.json")
    );
