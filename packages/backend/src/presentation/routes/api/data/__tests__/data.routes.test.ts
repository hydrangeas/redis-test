...
describe('Data Routes', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      container.register(DI_TOKENS.RateLimitLogRepository, {
        useValue: mockRateLimitLogRepository,
      });
  ...
  describe('GET /api/data/:path', () => {
    it('should retrieve data successfully with valid token', async () => {
      // モックの設定
      mockRateLimitUseCase.checkAndRecordAccess = vi.fn().mockResolvedValue(
        Result.ok({
          allowed: true,
          limit: 60,
          remaining: 59,
      ...
      console.log(
        'DataRetrievalUseCase was called:',
        mockDataRetrievalUseCase.retrieveData.mock.calls.length,
        'times',
      );
      if (mockDataRetrievalUseCase.retrieveData.mock.calls.length > 0) {
        console.log(
          'DataRetrievalUseCase calls:',
          mockDataRetrievalUseCase.retrieveData.mock.calls,
        );
      }
    ...
    it('should return 404 when data is not found', async () => {
      ...
      mockDataRetrievalUseCase.retrieveData = vi
        .fn()
        .mockResolvedValue(
