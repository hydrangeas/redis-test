...
import { ISecurityAuditService, SecurityEvent } from '@/domain/data/interfaces/secure-file-access.interface';
...
export class SecurityAuditService implements ISecurityAuditService {
  ...
  async logSecurityEvent(event: SecurityEvent): Promise<void> {
    try {
      ...
      if (event.type.includes('DENIED') || event.type.includes('ATTEMPT') || event.type.includes('EXCEEDED')) {
