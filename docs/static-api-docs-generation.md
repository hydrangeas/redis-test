...
async function generateOpenAPISpec() {
  ...
  // JSON形式で保存
  await fs.writeFile(path.join('dist', 'openapi.json'), JSON.stringify(spec, null, 2));

  // YAML形式でも保存（人間が読みやすい）
  await fs.writeFile(path.join('dist', 'openapi.yaml'), yaml.dump(spec));
...
async function buildStaticDocs() {
  try {
    ...
    // OpenAPI仕様書を埋め込む
    html = html.replace('OPENAPI_SPEC_PLACEHOLDER', JSON.stringify(spec));
