...
export class AuthHelper {
  ...
  async getStoredSession(context: BrowserContext): Promise<any> {
    ...
    // Supabaseセッションを取得
    const sessionData = localStorage.origins[0]?.localStorage.find(
...
test.describe('Logout Flow', () => {
  ...
  test('should clear all auth data on logout', async ({ page, context }) => {
    ...
    const authCookies = storageAfterLogout.cookies.filter(c => 
      c.name.includes('supabase') || c.name.includes('auth')
    );
    expect(authCookies.length).toBe(0);

    const authStorage = storageAfterLogout.origins[0]?.localStorage.filter(item =>
      item.name.includes('supabase') || item.name.includes('auth')
    );
