...
export class RateLimitLogService implements IRateLimitLogService {
  ...
  async checkAndLog(
    ...
    try {
      ...
      // 現在のウィンドウでのリクエスト数を取得
      const countResult = await this.getRequestCount(
        userId.value,
        endpoint,
        windowStart
      );
      ...
      this.logger.error({ error, userId: userId.value, endpoint }, 
  ...
  private async processBatch(): Promise<void> {
    ...
    await this.batchQueue.add(async () => {
      try {
        ...
        if (error) {
          this.logger.error({ error, logCount: logs.length }, 
            "Failed to batch insert rate limit logs");
          
          // 失敗したログを再度キューに戻す
          logs.forEach(log => this.addToBatch(log));
        } else {
          this.logger.debug({ logCount: logs.length }, 
...
export class RateLimitMetrics {
  ...
  recordRequest(
    ...
    this.windowSizeHistogram.observe(
      { endpoint, tier },
      windowSize
    );
  }
...
describe("RateLimitLogService Concurrency", () => {
  ...
  it("should handle concurrent requests correctly", async () => {
    ...
    for (let i = 0; i < concurrentRequests; i++) {
      promises.push(
        service.checkAndLog(userId, endpoint, tier)
      );
    }
