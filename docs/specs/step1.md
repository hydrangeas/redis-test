...
graph LR
    ...
    subgraph "Social Login認証フロー"
        ...
        A9[初回ログインが検出された🟧]
        A10[ユーザーアカウントが作成された🟧]
        A11[tier1がユーザーに割り当てられた🟧]
        
        %% 既存ユーザーのログイン
        A12[既存ユーザーのログインが確認された🟧]
    ...
    class A1,A2,A4,A6,A8,A9,A10,A11,A12,A13,A14,A15,A16,C1,C2,C3,C4,C5,C6,C7,D1,D2,D3,D4,D5,D6,E1,E2,E3,F1,F2,F4,G1,G2,G3,G4 event;
    class A5,F3 externalSystem;
```
...
- 認証がキャンセル/失敗した（ドメインイベント🟧）
  ユーザーが認証をキャンセルした、または認証プロバイダーでエラーが発生したイベント
