...
describe("APIAggregate", () => {
  ...
  describe("endpoint matching", () => {
    it("should match exact and wildcard endpoints", async () => {
      ...
      const result1 = await aggregate.checkAndRecordAccess(
        user,
        path1,
        method
      );
    ...
    it("should handle public endpoints without rate limiting", async () => {
      ...
      const result = await aggregate.checkAndRecordAccess(
        user,
        path,
        method
      );
  ...
  describe("concurrent access", () => {
    it("should handle concurrent requests correctly", async () => {
      ...
      for (let i = 0; i < 30; i++) {
        promises.push(
          aggregate.checkAndRecordAccess(user1, path, method)
        );
      }
      ...
      for (let i = 0; i < 30; i++) {
        promises.push(
          aggregate.checkAndRecordAccess(user2, path, method)
        );
      }
